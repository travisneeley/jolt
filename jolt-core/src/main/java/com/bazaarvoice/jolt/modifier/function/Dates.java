...
public class Dates {
    ...
    public static Optional<String> fromEpochMilli(Object arg, Object format) {
        ...
        return Optional.of(dateFormatter.format(Instant.ofEpochMilli(epoch)));
    }
    ...
    public static Optional<Long> toEpochMilli(Object date, Object format) {
        ...
        try {
            DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern(formatStr);
            return Optional.of(Instant.from(dateFormatter.parse(dateStr)).toEpochMilli());
        } catch (Exception ex) {
            throw new TransformException("Unable to format Date string into viable epoch", ex);
        }
    }
